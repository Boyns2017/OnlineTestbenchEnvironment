...
class Flag1(smach.State):
	...
	receivedflag1 = 0
	rospy.sleep(0.05)
	rospy.Subscriber("pick_up", Int8, callback1)
	if receivedflag1 == 1:
		receivedflag1 = 0
		return 'outcome1'
        else:
...
#	global stats
	receivedflag2 = 0
	rospy.Subscriber("pick_up", Int8, callback2)
	if receivedflag2 == 1:
...
def main(number):
	rospy.init_node('assertion7', anonymous=True) #Start node first
	global globaltime
